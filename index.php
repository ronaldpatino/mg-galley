					...
					// Set thumbnail to first image found (if any):
						...
						// If no folder.jpg or image is found, then display default icon:
							$dirs[] = array(
								...
								"html" => "<li class='thumbnail'>"
										. "<a href='?dir=" . ltrim($_GET['dir'] . "/" . $file, "/") . "'>" 
										. "<img src='" . GALLERY_ROOT . "images/folder_" . strtolower($folder_color) . ".png' width='$thumb_size' height='$thumb_size' alt='$label_loading' />"										
										. "<h5>". padstring($file, $label_max_length)."</h5>"
										. "</li>");
		        	...
		        	if ($extension != "")
			  		{
		  				$files[] = array (
							...
							"date" => filemtime($currentdir . "/" . $file),
							"size" => filesize($currentdir . "/" . $file),
			  				"html" => "<li  class='thumbnail'><a href='" . $currentdir . "/" . $file . "' title='$file'><em-pdf>" . padstring($file, 20) . "</em-pdf><span></span><img src='" . GALLERY_ROOT . "images/filetype_" . $extension . ".png' width='$thumb_size' height='$thumb_size' alt='$file' /></a></li>");
        			}
     			}   		
	}
  closedir($handle);
...
if (count($dirs) + count($files) == 0) {
	$thumbnails .= "<li  class='thumbnail'>$label_noimages</li>"; //Display 'no images' text
